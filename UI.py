...
def detect_disease():
    ...
    try:
        ...
        if selected_model == "ResNet50 Model":
            ...
            confidence = resnet_pred[0][resnet_label] * 100
            result_text = f"ResNet: {class_labels[resnet_label]} ({confidence:.2f}%)"

        elif selected_model == "CNN Model":
            cnn_pred = cnn_model.predict(img_cnn)
            cnn_label = np.argmax(cnn_pred)
            confidence = cnn_pred[0][cnn_label] * 100
            result_text = f"CNN: {class_labels[cnn_label]} ({confidence:.2f}%)"
